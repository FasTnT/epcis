...
namespace FasTnT.UnitTest.Queries.SimpleEventQuery
{
    ...
    public class WhenApplyingGE_eventTimeParameter : SimpleEventQueryTestBase
    {
        ...
        {
            EventFetcher.Verify(x => x.Apply(It.Is<ComparisonParameterFilter>(f => f.Comparator == FilterComparator.GreaterOrEqual && f.Field == EpcisField.EventTime && f.Value is DateTime)), Times.Once);
        }
    }
}
