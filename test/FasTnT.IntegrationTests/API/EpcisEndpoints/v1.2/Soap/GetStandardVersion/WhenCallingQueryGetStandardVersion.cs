...
namespace FasTnT.IntegrationTests.API.EpcisEndpoints.v1_2.Soap.GetStandardVersion
{
    ...
    public class WhenCallingQueryGetStandardVersion : BaseIntegrationTest
    {
        public override async Task Act()
        {
            ...
            Result = await Client.PostAsync("/v1_2/Query.svc", new StringContent(File.ReadAllText("Requests/Queries/GetStandardVersion.xml"), Encoding.UTF8, "application/xml"));
        }
        ...
        [Assert]
        public async Task ItShouldReturnTheStandardVersion1_2()
        {
            var content = await Result.Content.ReadAsStringAsync();
            var xmlDocument = XDocument.Parse(content);
