...
namespace FasTnT.IntegrationTests.API.EpcisEndpoints.v1_2.Soap.GetVendorVersion
{
    ...
    public class WhenCallingQueryGetQueryNames : BaseIntegrationTest
    {
        public override async Task Act()
        {
            ...
            Result = await Client.PostAsync("/v1_2/Query.svc", new StringContent(File.ReadAllText("Requests/Queries/GetQueryNames.xml"), Encoding.UTF8, "application/xml"));
        }
        ...
        [Assert]
        public async Task ItShouldReturnAtLeastTwoAvailableQueryNames()
        {
            var content = await Result.Content.ReadAsStringAsync();
            var xmlDocument = XDocument.Parse(content);
