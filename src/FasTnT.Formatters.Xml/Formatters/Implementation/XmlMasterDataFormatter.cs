...
namespace FasTnT.Parsers.Xml.Formatters.Implementation
{
    public static class XmlMasterDataFormatter
    {
        ...
        public static XElement Format(EpcisMasterData masterData) => new XElement("VocabularyElement", new XAttribute("id", masterData.Id), masterData.Attributes.Select(Format), FormatChildren(masterData.Children));
        private static XElement FormatChildren(List<string> children) => children.Any() ? new XElement("children", children.Select(x => new XElement("id", x))) : null;
